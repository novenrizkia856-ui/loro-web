...
| `token.address`, `token.symbol` | the project token, a separate contract from LoroLoan and LoroLens. The landing page's "Contract address" bar shows this address with a copy button; while `null` it reads "Coming soon". The footer shows the symbol |
